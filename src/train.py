...
class ProjectAgent:
    def __init__(self, save_path = None):
        ...
        self.save_path = save_path if save_path is not None else "model_Versini.pth"
    ...
    def load(self, path = None):
        ...
        if path is None:
            path = self.save_path
            # Depending on where the script is run, just in case
            if not os.path.isfile(path):
                path = "../" + self.save_path
            if not os.path.isfile(path):
                path = "mva-rl-assignment-lucas-versini/" + self.save_path
